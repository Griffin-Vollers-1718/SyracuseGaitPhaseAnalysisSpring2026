...
"""
GStrike = labeling.Label_Max(AXd1)
...
len_train = int(model_x_percent*len(AXd1))
X = AXd1[:len_train]
X = labeling.tran(X)
X_val = AXd1[len_train:len(AXd1)-200]
...
# Prepare data
train_loader, val_loader, val_dataset = pytorchNN.prepare_data(X, y, batch_size= batch_size)
...
predictions = pytorchNN.predict(model, X_val)
