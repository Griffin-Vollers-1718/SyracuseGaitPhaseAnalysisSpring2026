...
def load_data():
    ...
    # Rows = ['Angular Velocity X', 'Angular Velocity Y', 'Angular Velocity Z',
    #             'Linear Acceleration X', 'Linear Acceleration Y', 'Linear Acceleration Z' ]
    
    # Data_1 = pd.DataFrame(data1, index = Rows )
    # Time_1 = pd.DataFrame(time1, index = Rows)
    # Data_08 = pd.DataFrame(data08, index = Rows)
    # Time_08 = pd.DataFrame(time08, index = Rows)
    S1data_fsrgaitphase = np.reshape(S1data['gaitPhase']['data'][0,0],(18846,))
    S1data_fsrgait = pd.DataFrame(S1data_fsrgaitphase)
    S1data_fsrtime = np.reshape(S1data['gaitPhase']['time'][0,0],(18846,))
    S1data_fsrtime = pd.DataFrame(S1data_fsrtime)
    
    return Data_1, Time_1, Data_08, Time_08, S1data_fsrgait, S1data_fsrtime
