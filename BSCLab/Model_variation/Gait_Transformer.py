# This is where the Transformer Model will be made and created
# Will use the same architecure as the LSTM
...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, max_len=200, dropout=0.2):
        super().__init__()
...
class GaitTransformer(nn.Module):
    def __init__(self, input_dim, d_model=64, nhead=4, num_layers=2, output_dim=1):
        ...
        # Project (features + building embedding) into d_model
    ...
    def forward(self, x):
        ...
        x = self.pos_encoding(x)            # inject position info
...
def train_model(
    ...
    for epoch in range(num_epochs):
        ...
        for x_batch, y_batch in tqdm.tqdm(train_loader, desc=f'Epoch {epoch+1}'):
            x_batch = x_batch.to(device)
            y_batch = y_batch.to(device)
            y_batch = y_batch.float().squeeze(-1)  # Ensure shape is (batch, 1)
